        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {quickLinks.map(({ href, label, icon: Icon, desc }) => (
            <Link
              key={href}
              href={href}
              className="group flex flex-col gap-2 p-4 bg-gray-50 hover:bg-navy rounded-xl transition-colors cursor-pointer"
            >
              <div className="w-8 h-8 bg-white group-hover:bg-white/15 rounded-lg flex items-center justify-center transition-colors shadow-sm">
                <Icon size={15} className="text-navy group-hover:text-white transition-colors" />
              </div>
              <div>
