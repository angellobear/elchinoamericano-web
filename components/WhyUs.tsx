...
import { BadgeCheck, Layers3, PackageSearch, Truck } from "lucide-react"
  ...
  return (
    <section id="nosotros" className="bg-[#f6f8fb] py-21" aria-labelledby="about-title">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section header */}
        <div className="reveal-in flex flex-col items-center text-center gap-3 mb-14">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div className="reveal-in relative h-110 rounded-[20px] overflow-hidden border border-[#e6e9ef] shadow-[0_24px_50px_rgba(13,31,60,.12)] bg-[#eef1f6]">
            <Image
              src="/about-us.png"
          ...
          <div className="flex flex-col gap-3.5">
            {FEATURES.map(({ icon: Icon, title, description }) => (
              <div
                key={title}
                className="reveal-in flex gap-4 items-start bg-white border border-[#e6e9ef] rounded-[14px] py-5 px-5.5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(13,31,60,.08)]"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-12">
          {STATS.map(({ value, label }) => (
            <div key={label} className="reveal-in bg-navy rounded-2xl py-7.5 px-5 text-center">
